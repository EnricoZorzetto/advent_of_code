...
def get_neighbours(map, i, j):
    ny, nx = np.shape(map)
    YN = []; XN = []
    if i > 0: YN.append(i-1); XN.append(j)
    if i < ny-1: YN.append(i+1); XN.append(j)
    if j > 0: YN.append(i); XN.append(j-1)
    if j < nx-1: YN.append(i); XN.append(j+1)
    if j > 0 and i > 0: YN.append(i-1); XN.append(j-1)
    if j < nx-1 and i > 0: YN.append(i-1); XN.append(j+1)
    if j > 0 and i < ny-1: YN.append(i+1); XN.append(j-1)
    if j < nx-1 and i< ny-1: YN.append(i+1); XN.append(j+1)
    return len(YN), np.array(YN), np.array(XN)
